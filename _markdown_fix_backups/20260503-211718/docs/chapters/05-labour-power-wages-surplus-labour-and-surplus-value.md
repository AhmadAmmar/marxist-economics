# Labour-power, wages, surplus labour, and surplus value {#sec:labour-surplus}
...
## What the worker sells: labour-power, not labour {#sec:labourpower-not-labour}
...
#### A terminology map: surplus, surplus labour, surplus value, profit, exploitation {#sec:terms-surplus-profit}

Because these terms are often mixed up, fix the levels of analysis:

-   **Surplus (general):** output beyond what is immediately required to reproduce labour and social life at a given level (extra grain, stored food, extra cloth, extra labour-time, etc.). Surplus can exist in many kinds of society; the political question is *who controls it and on what basis* (see Section \[2.4\](../chapters/02-method-dialectical-and-historical-materialism.md#sec:sociallabour-surplus-control)).
...
-   **Surplus value ($s$):** the specifically capitalist form of surplus: the *value* (and therefore the money expression) created by surplus labour. It is produced in the labour process, because only living labour creates new value; but it must be *realised* through sale to appear in money-form (see Section \[6.2\](../chapters/06-constant-and-variable-capital-circuits-of-capital.md#subsec:circuits)).

-   **Profit ($\pi$):** the *money-form of realised surplus value* as it appears to the capitalist. In the simplest abstraction where commodities sell at their values, profit equals surplus value. But under competition and price formation, individual capitals can realise profits that differ from the surplus value they produce (see Section \[7.2\](../chapters/07-rate-of-profit-and-organic-composition-of-capital.md#sec:prices-production) and the discussion of $\pi_i - s_i$).
...
Let the working day be $T$ hours. Suppose the value of the worker's labour-power corresponds to $T_{\mathrm{N}}$ hours of labour---the *necessary* labour time. The remaining 
$$
T_{\mathrm{S}} = T - T_{\mathrm{N}},
  <a id="eq:Ts"></a>
$$
 is *surplus labour time*, during which the worker produces value for the capitalist without equivalent wages.

The new value created in the day is proportional to $T$, but the worker is only paid for $T_{\mathrm{N}}$. The surplus value is proportional to $T_{\mathrm{S}}$: 
$$
v \propto T_{\mathrm{N}},
  \qquad
  s \propto T_{\mathrm{S}}.
$$


We can therefore capture the rate of surplus value, or *rate of exploitation*, as 
$$
e = \frac{s}{v} = \frac{T_{\mathrm{S}}}{T_{\mathrm{N}}},
  <a id="eq:exploitation-time"></a>
$$
...
## Why surplus necessarily appears once labour-power is commodified {#sec:why-surplus}

Now we can state the logic in one chain.

Let the money-value created per hour of socially necessary labour be $\mu$ (money/hour). If the working day lasts $T$ hours, then the new value created in the day is: 
$$
\text{new value} = \mu \, T.
$$
 If the value of labour-power corresponds to $T_{\mathrm{N}}$ hours of socially necessary labour, then the wage (variable capital) corresponds to: 
$$
v = \mu \, T_{\mathrm{N}}.
$$
...
## Absolute and relative surplus value: the two main ways capital raises exploitation {#sec:absolute-relative-surplus}

Section \[5.3\](../chapters/05-labour-power-wages-surplus-labour-and-surplus-value.md#sec:why-surplus) established the baseline relation: 
$$
s = \mu\,T_S=\mu\,(T-T_N),
  \qquad
  v=\mu\,T_N,
...
### Absolute surplus value: extend (or densify) the working day {#absolute-surplus-value-extend-or-densify-the-working-day .unnumbered}

**Absolute surplus value** increases when surplus labour time rises because the working day is lengthened (or labour is intensified) while necessary labour time does not rise proportionally. In the simplest case, hold $T_N$ fixed and raise $T$: 
$$
\Delta T_N = 0,
  \qquad
  \Delta T > 0
  ...
  \Delta s = \mu\,\Delta T.
$$
 In concrete terms: longer shifts, forced overtime, fewer breaks, tighter supervision, speed-up, and piece-rates that compress the same tasks into less clock-time. When intensity rises, one can treat the hour itself as "denser" in value-creation terms (a higher effective $\mu$ for the same $T$), so surplus rises even without formally extending the day.

#### Minimal numeric example.

If $T=10$ hours and $T_N=5$, then $T_S=5$ and $e=5/5=100%$. If the day is extended to $T=12$ with $T_N$ still $5$, then $T_S=7$ and $e=7/5=140%$. Nothing "mystical" happened: the worker now performs more surplus labour time.

### Relative surplus value: shrink necessary labour time {#relative-surplus-value-shrink-necessary-labour-time .unnumbered}

**Relative surplus value** increases when surplus labour time rises because necessary labour time falls, with the working day held constant (or not rising by as much). Formally, hold $T$ fixed and reduce $T_N$: 
$$
\Delta T = 0,
  \qquad
  \Delta T_N < 0
...
If $T=10$ and $T_N=5$, then $T_S=5$ and $e=100%$. If productivity changes (in wage-goods) reduce $T_N$ to $4$ while $T$ remains $10$, then $T_S=6$ and $e=6/4=150%$.

#### Important clarification (often confused).

Relative surplus value is about *reducing the value of labour-power* (reducing $T_N$). That is not identical to *depressing wages below the value of labour-power*. The latter is an additional mechanism (common in informalisation, migrant labour regimes, and reproduction crises) where the worker is paid *less* than what is socially required for reproduction. Analytically, keep these distinct: one changes the value of labour-power; the other violates it.

### "Extra" surplus value and the competitive drive to revolutionise production {#extra-surplus-value-and-the-competitive-drive-to-revolutionise-production .unnumbered}
...
#### Why this matters for the rest of the pamphlet.

Absolute and relative surplus value are not abstract categories: they are the core grammar of class struggle under capitalism. They also connect directly to later sections: (1) countertendencies to falling profitability (raising $e$ by extending/intensifying labour or cutting the value of labour-power), and (2) the chronic pressure to hold wages down relative to productivity, which shapes crisis dynamics.

## Is this "theft"? Contract, coercion, and property relations {#sec:theft-or-relation}
...
-   therefore it cannot be abolished by moral appeals or contract reform alone, while the wage relation and private ownership of the main means of production remain.[^4]

This is exactly why the horizon of struggle is not only better terms inside the wage relation, but ultimately the abolition of the wage relation itself.[^5]

## Example: a coffee-shop worker
...
!!! example "Example: A coffee-shop worker"
    ...
    ### Step 1: Separate the value created by labour from the value of used-up machinery and inputs {#step-1-separate-the-value-created-by-labour-from-the-value-of-used-up-machinery-and-inputs .unnumbered}
    ...
    \begin{aligned}
      ...
      \text{Value of used-up constant capital } (c) &= $300,\\[0.5em]
      \Rightarrow\quad
    ...
    ### Step 2: Split this new value into wages and surplus-value {#step-2-split-this-new-value-into-wages-and-surplus-value .unnumbered}
    ...
    ### Step 3: Compute the rate of exploitation {#step-3-compute-the-rate-of-exploitation .unnumbered}
    ...
    ### Step 4: Translate the value relations into hours of the working day {#step-4-translate-the-value-relations-into-hours-of-the-working-day .unnumbered}
    ...
    \begin{aligned}
      \text{Necessary labour time}
        &= \frac{v}{v + s} \times 8\ \text{hours}\\[0.3em]
...
## Forms of wages: time-wages and piece-wages {#sec:wage-forms}
...
### Time-wages {#time-wages .unnumbered}
...
Time-wages make it appear as if the worker is paid for *all* labour performed in those hours. But in Marxian terms, the wage is the price of *labour-power*, not the price of *labour*. What is purchased is the worker's capacity to labour for $T$ hours under capital's control. The wage corresponds (in value terms) to $v$, the value of labour-power, i.e.Â the socially necessary labour time required to reproduce the worker.

To connect money-wages to labour-time, introduce the monetary expression of labour time (MELT), $\mu$ (money per hour of socially necessary labour time). Then the value of labour-power corresponds to a quantity of necessary labour time $T_{\mathrm{N}}$, and (schematically) 
$$
v = \mu \, T_{\mathrm{N}},
  \qquad
  W_w \approx v.
...
The approximation sign here flags that money-wages and market prices do not, in general, equal values. Section \[7.2\](../chapters/07-rate-of-profit-and-organic-composition-of-capital.md#sec:prices-production) explains how competition and the tendency toward an average rate of profit generate prices of production that systematically diverge from values; market power, state policy, and world-market (exchange-rate and terms-of-trade) conditions can produce further, conjunctural deviations. The key point is conceptual: even if $W_w$ equals the value of labour-power (no cheating), exploitation can still occur because the use-value of labour-power is value-creating labour for a longer working day than $T_{\mathrm{N}}$.

### Piece-wages {#piece-wages .unnumbered}
...
## Example: Sialkot football stitching {#subsec:sialkot-football-stitching}
...
!!! example "Example: Sialkot football stitching: piece-wage, implied time-wage, and the retail-price gap"
    []{#ex:sialkot-piecewage label="ex:sialkot-piecewage"}
    ...
    $$
     These figures are drawn from reporting on Sialkot football stitching in 2022 and are used here as an illustrative benchmark, not as a complete firm-level accounts sheet for a single year.[^6]
    ...
    Against the same report's quoted 2022 living-wage benchmark of about $\text{PKR }20{,}000$ per month, this is:
    ...
    Now compare this wage to a downstream retail benchmark. Suppose a branded match ball retails at $P_{\text{retail}}=\text{USD }165$.[^7]
    ...
    We return to the same industry in Example \[6.1\](../chapters/06-constant-and-variable-capital-circuits-of-capital.md#ex:sialkot-costtable), after introducing constant capital, variable capital, cost price, and producer-stage surplus.
...
#### Note on values and prices.

For simplicity in these numerical examples, treat observed money prices and costs as proportional to underlying values. In reality, prices can systematically deviate from values through competition and the redistribution of surplus value via prices of production; see Section \[7.2\](../chapters/07-rate-of-profit-and-organic-composition-of-capital.md#sec:prices-production). Market power, state policy, and world-market conditions can generate further deviations around these regulating prices. [^8]

Return to the smartphone example. Suppose a batch of phones sells for $1.4 million. The costs are:

-   $c = $1{,}000{,}000$ in components, depreciation, logistics.[^9]
...
$$
s = $1{,}400{,}000 - ($1{,}000{,}000 + $200{,}000) = $200{,}000.
$$
...
$$
e = \frac{s}{v}
  = \frac{$200{,}000}{$200{,}000} = 1 \quad\Rightarrow\quad e = 100%.
$$
...
First, this arithmetic treats money magnitudes as proportional to values for clarity. In reality, competition, productivity differentials, and market power generate systematic value--price divergences (see Section \[7.2\](../chapters/07-rate-of-profit-and-organic-composition-of-capital.md#sec:prices-production)); we flag this here but abstract from those divergences in the worked numbers. [^10]

Second, surplus value is produced where *productive labour* is set to work in production processes that create commodities (or commodity-services) for sale.[^11] Much of what happens "downstream" (advertising, branding, some retail functions, finance) is better analysed as the *realisation* and *redistribution* of surplus value rather than its direct production. The brand and its shareholders can nonetheless capture surplus generated across the global chain through control of intellectual property, market access, finance, logistics, and pricing power (including transfer pricing and monopoly rents).[41, 59]

[^1]: This is why Marx's argument does not depend on proving that every wage is "below subsistence" or that every contract is fraudulent. It is enough that workers must sell labour-power to live and that capital owns the product: once the working day exceeds necessary labour time, surplus labour (and thus surplus value) is produced even under fully "legal" payment.

[^2]: Put differently: the contract sets a price for labour-*power* (capacity), not for labour-*performed* (value created). The difference between the value created in the day and the wage paid is not a breach of contract; it is the normal outcome of capitalist production.

[^3]: This is the specific sense in which Marx calls the worker "free": free to sell labour-power to any capitalist, but also free of property in the means of production. The "choice" is therefore constrained by the material need to reproduce life under conditions of dispossession.

[^4]: Reforms matter for survival and bargaining power, and workers fight for them for good reason. The point is that reforms do not, by themselves, eliminate the structural production and appropriation of surplus so long as capital controls production and the product.

[^5]: In plain terms: ending exploitation requires breaking the link between survival and selling labour-power, and socialising the major means of production so that the surplus produced by collective labour is democratically planned and used rather than privately appropriated.

[^6]: Bloomberg Businessweek reported a piece-rate benchmark of roughly PKR $160$ per ball, about three hours per ball, roughly three balls per day, and an illustrative monthly wage figure of roughly PKR $9{,}600$; it also quotes a researcher-estimate living-wage benchmark of roughly PKR $20{,}000$ per month. See [55].

[^7]: For the retail-price benchmark, see [57]. The conversion below keeps the exchange-rate convention already used in this pamphlet: the NBP Treasury rate sheet for 20 January 2026, $1\,\text{USD} \approx \text{PKR }279.897$; see [66]. If one instead uses the Instagram-slide convention of roughly PKR $280.112$ per USD on 2 January 2026, the result changes only slightly.

[^8]: In what follows, this "proportional to values" convention should be read as: we treat the money figures as value magnitudes expressed in money (as if a roughly constant conversion from socially necessary labour time to money were operating), and we treat the consumed inputs as if they enter at their values. This isolates the logic of exploitation and accumulation. When one instead plugs in observed supplier prices, the same subtraction yields an *accounting residual* (realised profit for this firm/segment) that can mix surplus value produced here with surplus value redistributed through mark-ups, transfer pricing, monopoly rents, and credit conditions.

[^9]: Here $c$ is being treated as the *value transferred* by used-up inputs (and depreciation of fixed capital) into the output---i.e.Â as if these inputs were purchased at their values under the simplifying convention stated above. If instead we used observed input prices (supplier prices, contract prices, internal transfer prices), that figure could already include redistributed surplus value, so the residual computed below would not be a clean measure of surplus value *produced* by assembly labour alone.

[^10]: This matters *especially* for global value chains: the retail price of a branded device can sit far above the labour-time value embodied in the device because the price contains monopoly rents (IP, brand, platform control) and because surplus value is redistributed through supplier mark-ups, logistics contracts, and financial claims. The worked numbers should therefore be read as clarifying the logic of exploitation, not as a literal decomposition of the final retail price.

[^11]: "Unproductive" is a technical category here, not a moral one: it does not mean "useless" or "unimportant." It means labour that does not directly produce new value and surplus value for capital, even though it may be necessary for circulation, control, coordination, or the realisation and distribution of surplus.
