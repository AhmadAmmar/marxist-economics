# Run from the root of your local repo: C:\Users\Owner\marxist-economics
# This patch fixes the visible rendering errors in chapter 07 and chapter 16.

$ErrorActionPreference = "Stop"

if (!(Test-Path ".\mkdocs.yml")) {
    throw "Run this script from the repo root: C:\Users\Owner\marxist-economics"
}

# Remove accidental Pandoc warning lines from all generated Markdown files
Get-ChildItem ".\docs" -Recurse -File -Include *.md | ForEach-Object {
    $path = $_.FullName
    $text = Get-Content $path -Raw
    $text = $text -replace '(?m)^\[WARNING\].*\r?\n?', ''
    Set-Content -Path $path -Value $text -Encoding UTF8
}

# Fix chapter 16 macro-expansion loss
$ch16 = ".\docs\chapters\16-from-critique-to-transition-why-this-matters-for-you.md"
if (Test-Path $ch16) {
    $text = Get-Content $ch16 -Raw
    $text = $text.Replace(
        "The categories of value, surplus value, constant and variable capital, the , the , and the  are not abstract curiosities.",
        "The categories of value, surplus value, constant and variable capital, the profit rate ($r$), the organic composition of capital (OCC), and the tendency of the rate of profit to fall (TRPF) are not abstract curiosities."
    )
    $text = $text.Replace(
        "The categories of value, surplus value, constant and variable capital, the , the , and the are not abstract curiosities.",
        "The categories of value, surplus value, constant and variable capital, the profit rate ($r$), the organic composition of capital (OCC), and the tendency of the rate of profit to fall (TRPF) are not abstract curiosities."
    )
    Set-Content -Path $ch16 -Value $text -Encoding UTF8
}

# Replace chapter 07 with a hand-cleaned Markdown version
$chapter07 = @'
...
'@
Set-Content -Path ".\docs\chapters\07-rate-of-profit-and-organic-composition-of-capital.md" -Value $chapter07 -Encoding UTF8

# General cleanup: citations accidentally escaped as math delimiters in generated markdown.
Get-ChildItem ".\docs" -Recurse -File -Include *.md | ForEach-Object {
    $path = $_.FullName
    $text = Get-Content $path -Raw
    $text = $text -replace '\\\[([0-9][0-9,\s]*)\\\]', '[$1]'
    Set-Content -Path $path -Value $text -Encoding UTF8
}

Write-Host "Patch applied. Now run: python -m mkdocs build; python -m mkdocs serve"
