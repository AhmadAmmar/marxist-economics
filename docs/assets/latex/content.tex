...
The scope of this pamphlet cannot cover the entirety of \emph{Das Kapital}, but give you just enough
...
Marx did not start from ethics (\emph{capitalism is bad because it is unfair})
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Labour is simultaneously concrete activity (teaching, sewing, coding) and an abstract expenditure
...
every human society must solve: \emph{how life is reproduced}. People must eat, drink, rest,
...
Humans have always done this through \emph{social labour}. Not because individuals cannot ever
...
forms of obligation and reciprocity.\cite{LeeDaly1999} In other words, the basic unit is not the
...
This is why the commodity is not a neutral object. It is a \emph{social relation}. A commodity is a product (or service) produced for exchange
...
A \textbf{commodity} is, at minimum:

\begin{enumerate}[leftmargin=1.5em]
  \item a useful thing (\textbf{use-value}); and
  \item produced for exchange, not primarily for the direct use of its producer (\textbf{exchange-value}).
\end{enumerate}
...
When exchange-value is expressed in money, it appears as a \textbf{price}. Prices fluctuate with supply and demand,
...
of \emph{social labour}. To develop that claim, we now have to distinguish the \emph{appearance} of value in exchange
...
\subsubsection{Note: complex labour, skill, and the reduction to simple labour}
...
but the point is not that the wage \emph{creates} value; rather, it is one of the ways the system
\emph{registers} and reproduces these differences.\cite{Foley1986,Shaikh2016}
...
A simple way to see the point is to distinguish \emph{short-run fluctuations} from \emph{long-run discipline}. A failed
...
The ``law'' is not a moral law or a perfect measurement device. It is a \emph{tendential regulator}: competition reallocates production toward techniques closer to
...
weighted by socially realised output. This is why we formalise socially necessary labour time as an output-weighted average in Section~\ref{sec:snlt}: the techniques
...
individual values differ within a branch: the regulating magnitude is shaped by the conditions of production that prevail for the majority of output.\cite{Marx1894}}
...
Even if a firm expends labour-time, that labour is only \emph{validated} as socially necessary
...
they contain does not count as socially necessary \emph{for capital}, and the loss appears as
write-downs, layoffs, bankruptcies, and idle capacity.\cite{Marx1867}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Surplus labour (\(T_s\)):} in capitalism, the part of the working day performed beyond the labour
  required to reproduce the worker (beyond \emph{necessary} labour time \(T_n\)). This is a \emph{time} category:
...
\subsubsection*{Absolute surplus value: extend (or densify) the working day}

\textbf{Absolute surplus value} increases when surplus labour time rises because the working day
...
at the point of exchange. The wage contract can be formally voluntary and ``fair'' in the sense that the worker is paid the value of labour-power. The point, as shown in Section~\ref{sec:labourpower-not-labour}, is that exploitation is compatible with equivalent exchange because capital purchases control over labour-power for the whole working day, not the value the worker will create.\footnote{Put differently: the contract sets a price for labour-\emph{power} (capacity), not for labour-\emph{performed} (value created). The difference between the value created in the day and the wage paid is not a breach of contract; it is the normal outcome of capitalist production.}
...
benchmark, see \cite{GoalAlRihla2022}. The conversion below keeps the exchange-rate
...
\cite{NBPRateSheet20Jan2026}. If one instead uses the Instagram-slide convention
...
\footnote{In what follows, this ``proportional to values'' convention should be read as: we treat the money figures as value magnitudes expressed in money (as if a roughly constant conversion from socially necessary labour time to money were operating), and we treat the consumed inputs as if they enter at their values. This isolates the logic of exploitation and accumulation. When one instead plugs in observed supplier prices, the same subtraction yields an \emph{accounting residual} (realised profit for this firm/segment) that can mix surplus value produced here with surplus value redistributed through mark-ups, transfer pricing, monopoly rents, and credit conditions.}
...
\footnote{This matters \emph{especially} for global value chains: the retail price of a branded device can sit far above the
...
Example~\ref{ex:sialkot-piecewage}, but at a different level of abstraction.
...
\cite{AtkinEtAl2017}. The NBER working-paper version is
\cite{AtkinEtAl2015NBER}.}
...
This does \emph{not} mean that the full supply-chain profit on a branded football
...
has to be read with its construction in mind.\cite{Shaikh2016,Roberts2015}\footnote{In national accounts and firm accounts, what counts as ``profit'' can include or exclude interest, rents, taxes, capital gains, and various imputed items; the capital stock can be valued at historical or replacement cost; and depreciation can be treated in different ways. These choices shift levels and sometimes timing, but they do not remove the structural link Marx is emphasising between accumulation (rising capital intensity) and profitability pressures over time.}
...
so it does \emph{not} buy as much from Dept.\ II as before (a contraction in $I$).
...
and hard-currency debt service that tightens the balance of payments.\cite{IMFPakistan2023}
...
of \emph{devaluation and restructuring}: bankruptcies, write-downs, unemployment, wage
...
not only realise existing surplus but also realise an \emph{growing} mass of surplus each cycle.
...
intact.\cite{StrugglePK_WorldPerspectives,PeetUnholyTrinity}\footnote{This does not deny that real housing shortages, infrastructure deficits, and migration pressures exist. The point is that, under capitalist property relations, these conditions are routinely \emph{managed through price} and \emph{converted into rent opportunities}: scarcity and access become revenue streams. A ``housing crisis'' can therefore coexist with empty luxury units, gated enclaves, and land-banking, because profitability is organised around asset valuation and rent extraction rather than social need.}
...
The point is not that capitalism merely \emph{trades} globally, but that accumulation is organised and
...
while downstream claims concentrated in the core (branding, design/IP, finance, and retail control) dominate the realised price.\footnote{\textbf{Precision note:}
...
regime, and the price point used as the denominator.}\footnote{\textbf{Conceptual note:}
a small wage share of final retail price is \emph{not} itself a complete measure of exploitation (which depends on surplus relative to variable capital at the
point of production). It \emph{is} a clear indicator of how the \emph{realised} price of branded commodities is dominated by downstream claims (branding,
marketing, design/IP rents, finance, and retail markups) concentrated in the capitalist core. The argument here is therefore about \emph{value appropriation
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item most of the surplus value is realised and appropriated in the centres of finance, marketing, and intellectual property; and
...
The point is that once produced, the \emph{reproduction} cost of many digital goods is low, so profitability depends heavily
...
From a Marxian point of view, value is created wherever living labour is expended, but the \emph{distribution} of surplus value is shaped by power,
...
but because creditors and the world market enforce specific reproduction conditions.\cite{PeetUnholyTrinity,IMFPakistan2023}
...
turnover and accelerated AI workloads.\cite{IEADataCentres2019,Masanet2020,LiEtAl2023Thirsty}
