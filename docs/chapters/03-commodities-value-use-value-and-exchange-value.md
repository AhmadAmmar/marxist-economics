...
[leftmargin=1.5em]
  - a useful thing (**use-value}); and
  - produced for exchange, not primarily for the direct use of its producer (**exchange-value}).
...
meal produced in a commercial kitchen and sold through a delivery platform *is} a commodity. The
...
What matters is the *social form}: production for sale in a system where producers must secure their
...
metres of linen, one month of labour for a wage, one laptop for a certain sum of money. In capitalist societies,
this relation typically appears as a **money price}.
...
Capitalism generalises commodity exchange so that money becomes the *universal equivalent}: most goods and
services are routinely bought and sold, and survival itself depends on access to money. That is why under capitalism
the typical question is not only *what} is produced, but *what can be sold}, to whom, and for how much---
including the selling of labour-power for wages.

\paragraph{Price as the form of appearance.}
When exchange-value is expressed in money, it appears as a **price}. Prices fluctuate with supply and demand,
market power, speculation, and crises. Later (see Section~sec:prices-production) we return to why prices do not
systematically coincide with underlying values, even though they are not arbitrary.


## The question that leads to value



What interests Marx is: *what regulates these exchange ratios over the long run}? Why does a high-end smartphone
...
commensurability of commodities on the market points to a social common substance: not usefulness, but the organisation
of *social labour}. To develop that claim, we now have to distinguish the *appearance} of value in exchange
...
where:
  ...
  - $\LSN{i}$ is the *socially necessary labour time} required
        to produce one unit of commodity $i$: that is, the labour time needed
        under *average} conditions of production, with average skill and
        intensity, using the prevailing technology.
- $\MELT$ is the **monetary expression of labour time} (MELT): a proportionality
...
Section~sec:complex-labour-note for how this reduction is socially stabilised under capitalism.
...
### Note: complex labour, skill, and the reduction to simple labour
...
skill, training, and intensity; Marx treats *complex} (skilled) labour as
*multiplied simple labour}---that is, one hour of skilled labour counts as several
hours of average labour.[Marx1867]
...
hierarchies stabilise what counts as ``average'' labour in each branch over time. In capitalism,
these differences tend to express themselves through wage hierarchies and professional closure,
...
The key is to be clear about *what kind of object value is}. Many critics treat ``value'' as if it were a
natural substance like length or weight, and then declare a scandal when Marx uses labour-time. But Marx’s claim
is different and more specific: **value is a social relation that takes a quantitative form under generalised
commodity production}. The relevant question is not ``Can I see value directly on a price tag?'' The question is:
*what social process makes heterogeneous private labours comparable and forces production to conform to a social
average?}

\paragraph{Objection 1: ``Aren’t prices just supply, demand, and preferences?''}
Of course market prices move with supply and demand. Marx’s claim is different: those movements are *oscillations}
around regulating magnitudes anchored in production conditions and the allocation of social labour under competition.[Marx1867,Shaikh2016]

A simple way to see the point is to distinguish *short-run fluctuations} from *long-run discipline}. A failed
...
value of commodities over time, nor why competition systematically reallocates capital in search of an average rate of profit.[Marx1867,Shaikh2016]
Supply and demand explains *movement}; it does not explain the *centre of gravity} around which those movements occur
...
That commensuration is what the category of socially necessary labour time captures.[Marx1867]
...
under capitalism, *social labour is compelled to take a commensurable form}, and why labour-time is the relevant measure
...
Marx calls that denominator *abstract labour}. Value is the quantitative expression of that abstract social labour, and
**the unit of abstract labour is socially necessary labour time}: the time required *under average conditions}, with average
skill and normal intensity, using prevailing techniques.[Marx1867,Shaikh2016]
...
mere clock-time taken in isolation. It is time *normalised} by social averages of technique, intensity, and skill, and enforced by the
competitive discipline that compels producers to match prevailing conditions (see Section~sec:snlt).%
...
As clock-time, yes: an hour is an hour. As *abstract labour}, not necessarily.
Marx distinguishes **simple} (average) labour from **complex} (skilled) labour.
Complex labour counts as *multiplied} simple labour: one hour of skilled labour can count as several
hours of average labour.[Marx1867]

This is not a moral ranking of ``better'' people.
How that multiplication is socially produced (training pipelines, credential regimes, closure, wage hierarchies,
and uneven social reproduction costs) is discussed in Section~sec:complex-labour-note.[Foley1986,Shaikh2016]
...
and the loss appears as write-downs, layoffs, bankruptcies, and idle capacity.[Marx1867,Shaikh2016]
But this does *not} mean that ``prices determine value'' in an arbitrary or purely subjective way. It means that a society organised through private production
...
The ``law'' is not a moral law or a perfect measurement device. It is a *tendential regulator}: competition reallocates production toward techniques closer to
the social average and penalises producers whose unit labour-time is persistently above it (Section~sec:snlt). Over time, this pressure forms centres of
...
\paragraph{A concrete way to see this: weighted social averages.}
When different firms (or regions) produce the same commodity with different unit labour-times, the regulating social time is not a simple arithmetic mean. It is
weighted by socially realised output. This is why we formalise socially necessary labour time as an output-weighted average in Section~sec:snlt: the techniques
that supply the bulk of output dominate the social norm.\footnote{Marx develops a related idea in Volume~III when discussing how a ``market value'' can be formed when
individual values differ within a branch: the regulating magnitude is shaped by the conditions of production that prevail for the majority of output.[Marx1894]}

\paragraph{Objection 5: ``So is value measurable or not?''}
Value is not usually *directly observable} as a price tag in the way mass is observable on a scale. But that does not make it metaphysical. It makes it a social
...
disciplined; and competition generates a tendency toward average profit that redistributes surplus value (Section~sec:prices-production).[Marx1867,Shaikh2016]

This is also why we keep three distinct levels explicit later: **values}, **prices of production}, and **market prices} (Section~sec:prices-production).
...
[leftmargin=1.5em]
  - **``Profit is just mark-up.''} We separate surplus value (production) from profit (realisation and distribution)
  in Section~sec:labour-surplus and then show redistribution through prices of production in Section~sec:prices-production.
  - **``Nature creates value.''} We distinguish use-value (material wealth) from value (a capitalist social form) in
  Section~sec:nature-value.
  - **``Monopoly brands / rents disprove value theory.''} We show how monopoly power and rent are claims on surplus value,
  not new sources of it (see Sections~sec:prices-production and sec:fictitious-capital).




## Nature, wealth, and the source of value
...
[leftmargin=1.5em]
  - **Nature is indispensable to production}: it provides materials, energy flows,
  ecosystems, and conditions of life. Without nature there is no labour process and no material wealth.
  - **Nature is a source of use-values (material wealth)}: some use-values exist with little or
  ...
  regulated by **socially necessary labour time} and expressed in money.
...
labour-time that appears *as value} in the dominant social accounting of wealth.

This distinction matters because capitalism can *increase} physical throughput and ecological harm
while *reducing} the value embodied per unit (if productivity rises and \snlt{} falls). In other words:
...
[leftmargin=1.5em]
  ...
  - The total wage bill for assembly workers per phone might be on the
  order of \pounds10--\pounds15.
  - The rest of the price covers constant capital (components, machinery
