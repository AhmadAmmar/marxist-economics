...
[^9]: Here $c$ is being treated as the *value transferred* by used-up inputs (and depreciation of fixed capital) into the output---i.e. as if these inputs were purchased at their values under the simplifying convention stated above. If instead we used observed input prices (supplier prices, contract prices, internal transfer prices), that figure could already include redistributed surplus value, so the residual computed below would not be a clean measure of surplus value *produced* by assembly labour alone.
