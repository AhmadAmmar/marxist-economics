...
[leftmargin=1.5em]
  - **Labour-power} is the worker's capacity to work, treated as a
  ...
  support.
  - **Labour} is the actual activity performed during the working
  day.
...
So the contract is for *labour-power}, not labour itself. This distinction is the
...
*necessary labour} (corresponding to $v$), while the remaining portion is
*surplus value} $s$, the money-form of unpaid surplus labour time.
...
[leftmargin=1.5em]
  - **Surplus (general):} output beyond what is immediately required to reproduce labour and social life
  at a given level (extra grain, stored food, extra cloth, extra labour-time, etc.). Surplus can exist in many
  kinds of society; the political question is *who controls it and on what basis} (see Section~sec:sociallabour-surplus-control). 

  - **Surplus labour (\(T_s\)):} in capitalism, the part of the working day performed beyond the labour
  required to reproduce the worker (beyond *necessary} labour time \(T_n\)). This is a *time} category:
  it refers to unpaid labour-time.

  - **Surplus value (\(s\)):} the specifically capitalist form of surplus: the *value} (and therefore
  the money expression) created by surplus labour. It is produced in the labour process, because only living labour
  creates new value; but it must be *realised} through sale to appear in money-form (see Section~subsec:circuits).

  - **Profit (\(\pi\)):} the *money-form of realised surplus value} as it appears to the capitalist.
  ...
  produce (see Section~sec:prices-production and the discussion of \(\pi_i - s_i\)).

  - **Exploitation (\(e\)):} the social relation in which the producing class performs surplus labour and a
...
\noindent
**Two common confusions to avoid.} (1) Profit is not explained by a general ``mark-up'': if one seller sells
...
## Why surplus necessarily appears once labour-power is commodified
...
cannot explain profit *as a normal outcome of equivalent exchange}.


## Absolute and relative surplus value: the two main ways capital raises exploitation



Section~sec:why-surplus established the baseline relation:
\[
  s = \mu\,T_S=\mu\,(T-T_N),
  \qquad
  v=\mu\,T_N,
...
*can} be produced, but how to raise it. There are two structurally distinct levers.


### Absolute surplus value: extend (or densify) the working day


**Absolute surplus value} increases when surplus labour time rises because the working day
is lengthened (or labour is intensified) while necessary labour time does not rise proportionally.
In the simplest case, hold $T_N$ fixed and raise $T$:
\[
  \Delta T_N = 0,
  \qquad
  \Delta T > 0
  ...
  \Delta s = \mu\,\Delta T.
\]
In concrete terms: longer shifts, forced overtime, fewer breaks, tighter supervision, speed-up,
and piece-rates that compress the same tasks into less clock-time. When intensity rises, one can
treat the hour itself as “denser” in value-creation terms (a higher effective $\mu$ for the same $T$),
so surplus rises even without formally extending the day.
...
**Relative surplus value} increases when surplus labour time rises because necessary labour
...
the *value} of that bundle can fall. Then the value of labour-power falls, and the portion of
...
\paragraph{Important clarification (often confused).}
Relative surplus value is about *reducing the value of labour-power} (reducing $T_N$).
That is not identical to *depressing wages below the value of labour-power}.
The latter is an additional mechanism (common in informalisation, migrant labour regimes, and
reproduction crises) where the worker is paid *less} than what is socially required for reproduction.
...
An individual firm can momentarily gain **extra surplus value} by introducing a technique that
...
system-wide route to *relative} surplus value.

\paragraph{Why this matters for the rest of the pamphlet.}
Absolute and relative surplus value are not abstract categories: they are the core grammar of class
...
[leftmargin=1.5em]
  ...
  - therefore it cannot be abolished by moral appeals or contract reform alone,
        while the wage relation and private ownership of the main means of production remain.\footnote{Reforms matter for survival and bargaining power, and workers fight for them for good reason. The point is that reforms do not, by themselves, eliminate the structural production and appropriation of surplus so long as capital controls production and the product.}
...
[leftmargin=1.5em]
  ...
  - Of this, \$300 covers the value of used-up constant capital (beans, milk,
        cups, energy, machine depreciation) --- this is $\constcap$.
  - The barista's wage bill for the shift is \$100 --- this is $\varcap$.


We want to find:

[leftmargin=1.5em]
  - the new value created by living labour during the shift ($\varcap + \surplus$),
  - the surplus value $\surplus$,
  - the rate of exploitation $\exploitrate$, and
  - how many hours of the 8-hour day are necessary labour time and how many
...
\]
So, of the *new} value (\$500) created in the day, one-fifth belongs to
...
[leftmargin=1.5em]
  - In the first $1.6$ hours of the shift, the barista produces value
        equivalent to their own wage (necessary labour time).
  - In the remaining $6.4$ hours, they produce surplus-value for the owner
        (surplus labour time).
  - Over the whole day, the rate of exploitation is $\exploitrate = 400%$:
        for every \$1 paid in wages, \$4 of surplus-value is extracted.


This does *not} mean that the branch manager personally pockets
...
Then the *money wage bill} is
\[
  W_w = w_h \, T.
\]

Time-wages make it appear as if the worker is paid for *all} labour performed in
those hours. But in Marxian terms, the wage is the price of *labour-power}, not the
price of *labour}. What is purchased is the worker's capacity to labour for $T$
...
*time-wages in disguise}. The piece-rate is derived from an assumed *normal} output
...
a *modified} time-wage in which the wage is tied more directly to labour intensity and output.
...
how exploitation is *experienced}, enforced, and fought over; they do not abolish
...
roughly PKR \(20{,}000\) per month. See [Bloomberg2022Sialkot].}
...
Now compare this wage to a downstream retail benchmark. Suppose a branded match
ball retails at \(P_{\text{retail}}=\text{USD }165\).\footnote{For the retail-price
benchmark, see [GoalAlRihla2022]. The conversion below keeps the exchange-rate
convention already used in this pamphlet: the NBP Treasury rate sheet for
20 January 2026, \(1\,\text{USD} \approx \text{PKR }279.897\); see
[NBPRateSheet20Jan2026]. If one instead uses the Instagram-slide convention
of roughly PKR \(280.112\) per USD on 2 January 2026, the result changes only
slightly.}
...
This calculation does *not} mean that the local employer pockets the full
...
We return to the same industry in Example~ex:sialkot-costtable, after
...
``efficiency''. Atkin et al.'s study of Sialkot football producers examined a new
...
\footnote{In what follows, this ``proportional to values'' convention should be read as: we treat the money figures as value magnitudes expressed in money (as if a roughly constant conversion from socially necessary labour time to money were operating), and we treat the consumed inputs as if they enter at their values. This isolates the logic of exploitation and accumulation. When one instead plugs in observed supplier prices, the same subtraction yields an *accounting residual} (realised profit for this firm/segment) that can mix surplus value produced here with surplus value redistributed through mark-ups, transfer pricing, monopoly rents, and credit conditions.}

Return to the smartphone example. Suppose a batch of phones sells for
\$1.4 million. The costs are:

[leftmargin=1.5em]
  - $\constcap = \$1{,}000{,}000$ in components, depreciation, logistics.\footnote{Here $\constcap$ is being treated as the *value transferred} by used-up inputs (and depreciation of fixed capital) into the output---i.e.\ as if these inputs were purchased at their values under the simplifying convention stated above. If instead we used observed input prices (supplier prices, contract prices, internal transfer prices), that figure could already include redistributed surplus value, so the residual computed below would not be a clean measure of surplus value *produced} by assembly labour alone.}
...
the firm's *realised profit} on the transaction, which can diverge from surplus value produced in the assembly segment
...
$$
This is a *toy} measure that treats the wage bill $\varcap$ as the variable capital advanced by the assembly employer and
...
\footnote{This matters *especially} for global value chains: the retail price of a branded device can sit far above the
...
processes that create commodities (or commodity-services) for sale.\footnote{``Unproductive'' is a technical category here, not a moral one: it does not mean ``useless'' or ``unimportant.'' It means labour that does not directly produce new value and surplus value for capital, even though it may be necessary for circulation, control, coordination, or the realisation and distribution of surplus.}
