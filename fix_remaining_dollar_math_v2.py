...
def targeted_chapter05(text: str) -> str:
    # Use lambda replacement so Python's re engine does not treat \frac / \mathrm
    # in the replacement text as invalid replacement escapes.
    fixed = r"as $e = \frac{s}{v} = \frac{T_{\mathrm{S}}}{T_{\mathrm{N}}}$, because"
    ...
    # Simpler literal fallback for the exact screenshot text after normalisation.
    text = text.replace(
