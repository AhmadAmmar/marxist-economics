...
% Robust URL breaking (helps long DOIs/URLs in manual bibliography)
\Urlmuskip=0mu plus 1mu\relax
\usepackage[margin=2.5cm]{geometry}
% Last-resort line-break slack for occasional long-link paragraphs
...
\hypersetup{
  colorlinks = true,
  breaklinks = true,          % allow link wrapping
...
% Improves PDF bookmarks robustness with hyperref
\usepackage{bookmark}

\usepackage[capitalise]{cleveref}

% ----------------------------------------------------------------------
% Strict integrity guard: treat undefined references/citations as errors.
% NOTE: On a first-pass compile (before labels/citations settle), LaTeX may
% legitimately report undefined refs/cites. Re-run after fixing/settling.
% ----------------------------------------------------------------------
\makeatletter
\def\@refundefined{%
  \ifG@refundefined
    \PackageError{pamphlet}{There were undefined references or citations}{%
      Fix missing \string\label/\string\ref/\string\cref targets and missing \string\bibitem keys.\MessageBreak
      If this is an initial compilation pass, rerun LaTeX once the .aux is populated.}%
  \fi}
\makeatother
...
% ----------------------------------------------------------------------
% Defensive fallbacks for glossaries-extra internals.
% These prevent compile failures on some TeX distributions / versions where
% the style hooks may be executed before these commands are defined.
% ----------------------------------------------------------------------
\providecommand*{\glossaryentryfield}[5]{}
\providecommand*{\glossarysubentryfield}[6]{}
...
% Give double-digit subsection numbers enough room in the table of contents
% so entries like "10.10 Reforms..." do not run into the title.
\makeatletter
...
% ----------------------------------------------------------------------
% Optional development-time integrity checks (leave commented for release)
% Uncomment to surface undefined refs/cites and unused labels in the log.
% ----------------------------------------------------------------------
% \usepackage{refcheck}
