...
$d$ &
discount rate used for present-value calculations (e.g.\ in the fictitious-capital
formula); depending on context it may incorporate interest, risk, and liquidity. \\
...
$w_i$ &
value of the output of industry $i$ (at the level of value magnitudes): $w_i = c_i + v_i + s_i$ (see Section~\ref{sec:prices-production}). \\

$c$ &
\textbf{constant capital}: in \(W=c+v+s\), the value transferred to the product from used-up materials, energy, and depreciation of fixed capital; it does \emph{not} create new value. In the simplified profit-rate equations, \(c\) also stands for constant capital advanced under the one-turnover convention used in the pamphlet. \\
...
$\mathrm{OCC}$ &
\textbf{organic composition of capital}: for expository simplicity, this pamphlet uses \(\mathrm{OCC}=c/v\) as shorthand. Strictly speaking, \(c/v\) is the value composition of capital; it is organic insofar as it reflects changes in the underlying technical composition of production. \\

$\Omega$ &
average organic composition of capital (in the prices-of-production example): $\Omega = \dfrac{C}{V}$ where $C=\sum_i c_i$ and $V=\sum_i v_i$ (see Section~\ref{sec:prices-production}). \\
...
$q_{ij}$ &
socially realised output of commodity \(i\) supplied by firm \(j\) during the period, used as an illustrative weight in Eq.~\eqref{eq:snlt}. The weighted average is a pedagogical approximation, not a general definition of socially necessary labour time. \\
...
$k_i$ &
cost price in industry $i$: $k_i = c_i + v_i$ (capital advanced that must be replaced). \\

$p_i$ &
\textbf{price of production} in industry $i$: regulating price yielding the average profit rate, $p_i = k_i(1+r)$ (see Section~\ref{sec:prices-production}). \\

$\pi_i$ &
profit received in industry $i$ under prices of production: $\pi_i = p_i - k_i = r\,k_i$. \\
